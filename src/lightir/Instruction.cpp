...
CallInst::CallInst(Function *func, std::vector<Value *> args, BasicBlock *bb)
    ...
    for (unsigned i = 0; i < args.size(); i++) {
        auto functype = func_type->get_param_type(i);
        auto argtype = args[i]->get_type();
        assert(functype == argtype &&
...
Type *GetElementPtrInst::get_element_type(Value *ptr,
    ...
    assert(
        ...
        (ty->is_array_type() || ty->is_integer_type() || ty->is_float_type() || ty->is_pointer_type()));
    if (ty->is_array_type()) {
